...
class OutputOnlyModel(torch.nn.Module):
    ...
    def __init__(self, model: AutoModel, encoder_name: str | List[str] = "encoder"):
        ...
        self.encoder_name = encoder_name if isinstance(encoder_name, list) else [encoder_name]
        self.device = (
            model.device
        ...
        self.encoder = torch.nn.ModuleList([model.base_model.get_submodule(encoder_name)]) if isinstance(encoder_name, str) else torch.nn.ModuleList([model.base_model.get_submodule(n) for n in encoder_name])
    ...
    def forward(
        ...
        if only_max_pred_id:
            self.lm_head.register_forward_hook(self._max_hook)
        ...
        if select is not None:
            ...
            y = torch.gather(y, dim=-1, index=select[:, pred_id, :])
...
@torch.no_grad()
def estimate_pdet(
    eigenvalues: torch.Tensor, det_regularization: float = 1e-3
) -> torch.Tensor:
    ...
    log_lb = torch.sum(torch.log(eigenvalues + det_regularization), dim=-1) - (
        d - rk_est
    ) * math.log(det_regularization)

    log_ub = (
        log_lb
        + det_regularization * tr_rad_ratio
        + (1 - det_regularization) * tr_sq_rad_ratio / 2
    )
...
def pullback(
    ...
    assert (
        input_simec.shape[0] % 3 == 0
    ), f"Batch size {input_simec.shape[0]} must be divisible by 3"

    #########################################################
    jac = (
        torch.stack(
            [
                torch.cat([chunk, chunk[-1:]], dim=0)
    ...
    with torch.no_grad():
        ...
        try:
            pullback_metric = pullback_metric.reshape(
                output_batch_size, max_pred_ids, max_len, *pullback_metric.shape[-2:]
            )
        except RuntimeError:
            pullback_metric = pullback_metric.reshape(
                2 * output_batch_size,
        ...
        if return_trace:
            ...
            special_pdets = estimate_pdet(
                eigenvalues, det_regularization=det_regularization
            )
